 ...
 */
 ...
 /* INTERNAL CODE FOR ASSIGNMENT 1*/
 static void update_openfile_count(void){
     struct pid_node *temp;
     spin_lock(&monitored_list_lock);
     list_for_each_entry(temp, &monitored_list, list){
         if (temp->proc_resource->pid == current->pid){
             temp->proc_resource->openfile_count++;
             break; //PROCESS FOUND NO NEED TO ITERATE FURTHER
         }
     }
 ...
 SYSCALL_DEFINE3(open, const char __user *, filename, int, flags, umode_t, mode)
 {
     if (force_o_largefile())
         flags |= O_LARGEFILE;
     long fd = do_sys_open(AT_FDCWD, filename, flags, mode);
     if (fd >= 0){
         update_openfile_count();
     }
     return fd;
 }
         ...
         umode_t, mode)
 {
     if (force_o_largefile())
         flags |= O_LARGEFILE;
     long fd = do_sys_open(dfd, filename, flags, mode);
     if (fd >= 0){
         update_openfile_count();
     }
     return fd;
         ...
         struct open_how __user *, how, size_t, usize)
 {
     ...
     long fd = do_sys_openat2(dfd, filename, &tmp);
     if (fd >= 0){
         update_openfile_count();
     }
     return fd;
