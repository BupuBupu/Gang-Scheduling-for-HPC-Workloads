 ...
 */
 ...
 /*
 SYSCALLS FOR ASSIGNMENT 1
 */
 LIST_HEAD(monitored_list);
 DEFINE_SPINLOCK(monitored_list_lock);
 
 static int __init init_monitored_list(void)
 {
     spin_lock_init(&monitored_list_lock);
     return 0;
 }
 
 module_init(init_monitored_list);
 ...
 SYSCALL_DEFINE1(register, pid_t, pid){
     ...
     if (!task){
         return -3; //PID Does not exist
     }
 
     //To check if already in list
     list_for_each_entry(temp, &monitored_list, list){
         if (temp->proc_resource->pid == pid){
             return -23; //Already monitored
         }
     }
     ...
     new_node->proc_resource = kmalloc(sizeof(struct per_proc_resource), GFP_KERNEL);
 
     if (!new_node->proc_resource) {
         kfree(new_node);
         return -ENOMEM;
     }
     ...
     spin_lock(&monitored_list_lock);
     list_add_tail(&new_node->list, &monitored_list);
     spin_unlock(&monitored_list_lock);
     return 0;
 }
 ...
 SYSCALL_DEFINE2(fetch, struct per_proc_resource __user *, stats, pid_t, pid) {
     ...
     if (!task1) {
         return -22; // PID not found
     }
     spin_lock(&monitored_list_lock);
     list_for_each_entry(node, &monitored_list, list) {
         if (node->proc_resource->pid == pid) {
             task = find_task_by_vpid(pid);
             heapsize = node->proc_resource->heapsize;
             //Update the heapsize 
             node->proc_resource->heapsize = heapsize >> 20; //IN MB
             
             if (copy_to_user(stats, node->proc_resource, sizeof(struct per_proc_resource))){
                 spin_unlock(&monitored_list_lock);
                 return -EFAULT;
             }
